...
function scriptcrown_ftn_optionsframework_add_admin(){
	global $query_string;
	$themename=scriptcrown_ftn_get_option('of_themename');
	$shortname=scriptcrown_ftn_get_option('of_shortname');
	if(isset($_REQUEST['page'])&& $_REQUEST['page']=='optionsframework'){
		if(isset($_REQUEST['of_save'])&& 'reset'==$_REQUEST['of_save']){
			$options=scriptcrown_ftn_get_option('of_template');
			scriptcrown_ftn_reset_options($options,'optionsframework');
			header("Location: admin.php?page=optionsframework&reset=true");
			die;
			}
		}
	$of_page=add_theme_page($themename,'Option Panel','edit_theme_options','optionsframework','scriptcrown_ftn_optionsframework_options_page','div');
	// Add framework functionaily to the head individually
	add_action("admin_print_scripts-$of_page",'scriptcrown_ftn_load_only');
	}
add_action('admin_menu','scriptcrown_ftn_optionsframework_add_admin');
/* ----------------------------------------------------------------------------------- */
/* Options Framework Reset Function - of_reset_options */
/* ----------------------------------------------------------------------------------- */
function scriptcrown_ftn_reset_options($options,$page=''){
	global $wpdb;
	$count=0;
		...
		if(isset($option['id'])){
			...
			if($option_type=='multicheck'){
				foreach($option['options'] as $option_key=> $option_option){
					scriptcrown_ftn_delete_option("{$option_id}_{$option_key}");
					}
				}else if(is_array($option_type)){
				foreach($option_type as $inner_option){
					$option_id=$inner_option['id'];
					scriptcrown_ftn_delete_option($option_id);
					}
				}else{
				scriptcrown_ftn_delete_option($option_id);
				}
	...
	if($page=='optionsframework'){
		scriptcrown_ftn_delete_option('of_options');
		}
	}
/* ----------------------------------------------------------------------------------- */
/* Build the Options Page - optionsframework_options_page */
/* ----------------------------------------------------------------------------------- */
function scriptcrown_ftn_optionsframework_options_page(){
	$options=scriptcrown_ftn_get_option('of_template');
	$themename=scriptcrown_ftn_get_option('of_themename');
	?>
    ...
    $return=scriptcrown_ftn_optionsframework_machine($options);
    ?>
        <div id="main">
          <div id="of-nav">
            <ul>
...
/* ----------------------------------------------------------------------------------- */
function scriptcrown_ftn_load_only(){
	add_action('admin_head','scriptcrown_ftn_admin_head');
...
function scriptcrown_ftn_admin_head(){
	// COLOR Picker ?>
	...
	jQuery(document).ready(function(){
		...
		jQuery('.of-check').click(function() {
			if(this.checked){
				jQuery(this).val(1);
			} else {
				jQuery(this).val(0);
		...
		foreach($options as $option){
			if($option['type']=='color' OR $option['type']=='typography' OR $option['type']=='border'){
				if($option['type']=='typography' OR $option['type']=='border'){
					$option_id=$option['id'];
					$temp_color=scriptcrown_ftn_get_option($option_id);
					$option_id=$option['id'] . '_color';
					$color=$temp_color['color'];
					}else{
					$option_id=$option['id'];
					$color=scriptcrown_ftn_get_option($option_id);
					}
				?>
...
<?php }
/* ----------------------------------------------------------------------------------- */
/* Ajax Save Action - scriptcrown_ftn_ajax_callback */
/* ----------------------------------------------------------------------------------- */
add_action('wp_ajax_of_ajax_post_action','scriptcrown_ftn_ajax_callback');
function scriptcrown_ftn_ajax_callback(){
	...
	if($save_type=='upload'){
		...
		scriptcrown_ftn_update_option($clickedID,$uploaded_file['url']);
		...
		scriptcrown_ftn_delete_option($id);
		...
		foreach($options as $option_array){
			$id=$option_array['id'];
			$old_value=scriptcrown_ftn_get_option($id);
			$new_value='';
			if(isset($output[$id])){
				...
				if(is_array($type)){
					foreach($type as $array){
						if($array['type']=='text'){
							...
							scriptcrown_ftn_update_option($id,stripslashes($new_value));
							}
						}
...
/* ----------------------------------------------------------------------------------- */
function scriptcrown_ftn_optionsframework_machine($options){
	$counter=0;
	$menu='';
	$output='';
	foreach($options as $value){
		...
		switch($value['type']){
			...
			case 'text':
			$val=$value['std'];
			$std=scriptcrown_ftn_get_option($value['id']);
			if($std !=""){
				$val=$std;
				}
			...
			$chk = ($val==1)? 'checked':'';	
			$output .='<input class="of-check" '.$chk.' name="' . $value['id'] . '" id="' . $value['id'] . '" type="' . $value['type'] . '" value="' . $val . '" />';
			break;
			/******************/
			case 'number':
			$val=$value['std'];
			...
			$value['std']='';
			if(isset($value['std'])){
				$output .=scriptcrown_ftn_optionsframework_uploader_function($value['id'],$value['std'],null);
				}
			break;
			/******************/
			case "upload_min":
			$output .=scriptcrown_ftn_optionsframework_uploader_function($value['id'],$value['std'],'min');
			break;
			/******************/
			case "color":
			$val=$value['std'];
			$stored=scriptcrown_ftn_get_option($value['id']);
			if($stored !=""){
				$val=$stored;
		...
		if(is_array($value['type'])){
			foreach($value['type'] as $array){
				$id=$array['id'];
				$std=$array['std'];
				$saved_std=scriptcrown_ftn_get_option($id);
				if($saved_std !=$std){
					$std=$saved_std;
					}
				$meta=$array['meta'];
...
/* ----------------------------------------------------------------------------------- */
/* OptionsFramework Uploader - scriptcrown_ftn_optionsframework_uploader_function */
/* ----------------------------------------------------------------------------------- */
function scriptcrown_ftn_optionsframework_uploader_function($id,$std,$mod){
	$uploader='';
	$upload=scriptcrown_ftn_get_option($id);
	if($mod !='min'){
		$val=$std;
		if(scriptcrown_ftn_get_option($id)!=""){
			$val=scriptcrown_ftn_get_option($id);
			}
